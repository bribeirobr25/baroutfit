...
4. **UI quando há `brandMatch` e a banda computada fica abaixo do tier editorial** — ⏭️ DIFERIDA para Fase B → ✅ **FECHADA (2026-06-15)**. Implementada como **referência verificada** (não "a banda cede ao tier"): match por produto (`matchAuditedProduct`), `BrandMatch.reference` no contrato, e um bloco na UI que distingue banda computada (página) × specs "verificado na fonte" (fato) × tier "nossa avaliação" (julgamento). Plano: `docs/plans/fase-b-decisao-4-kb-verificada.md`. A premissa original ("BrandMatch sem tier, match por host") foi resolvida construindo exatamente isso.
