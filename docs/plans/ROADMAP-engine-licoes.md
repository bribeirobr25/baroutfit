...
7. **Nunca inventar QUANTIDADE, só valores.** Cardinalidade é dado tanto quanto um GSM ou uma fibra. Variantes de tamanho da mesma foto **não** são várias fotos; N linhas de spec repetidas **não** são N specs. Deduplicar por **identidade** é a guarda do "não inventar quantidade", paralela ao "não inventar valor". **Mais linhas ≠ mais verdade** — vale para imagens, specs, achados (origem: G1, 2026-06-17; ver §6/M5 sobre como quase não vimos isso).
...
**M5 — Confie no HARNESS, não só no código (de G1, 2026-06-17).** Gastei ~6 iterações achando que o dedup estava quebrado porque o servidor ao vivo devolvia "2" — quando um teste unit **e** uma prova em node diziam "1". Causa: um **servidor zumbi na :3100 servindo código velho** (o `pkill -f` não casou o processo). Lição: **quando um teste hermético passa mas o check ao vivo discorda, suspeite do RUNTIME/ambiente antes da lógica.** É a mesma família do M1 (disco≠deployado): lá "prod roda código velho", aqui "meu check local roda código velho". *Conserto prático:* matar por **porta** (`lsof -ti:PORT | xargs kill`), não por nome; e a guarda estrutural é OPS-2.
...
**OPS-2 — carimbo de versão (materializa M1 + M5) · ✅ feito (2026-06-17).** `GET /api/health` → `{status, version, ref}` (version = `VERCEL_GIT_COMMIT_SHA` → `APP_VERSION` → `"dev"`; `no-store`). `pnpm smoke` imprime a versão do build vivo e **falha em mismatch** quando `EXPECTED_SHA` é passado. **Um único sinal que pega os dois bugs** — o de prod (M1) e o do zumbi local (M5) — na hora. Uso pós-deploy: `EXPECTED_SHA=$(git rev-parse HEAD) pnpm smoke`.

**Sequenciamento:** OPS-1 é quase de graça e **precede** o envio do trabalho de imagem não-commitado. OPS-2 é pequeno e paga-se no primeiro "por que isto não muda?". M2/M3/M4/M5 **não viram workstreams novos** — entram como itens/hábitos nos P2/P3/P5/OPS existentes.
...
- **P2.4** deixa de ser "rótulos de proveniência na UI" e passa a ser **mudança de forma do pipeline** (extrator emite `{value, source, scope}`; parser consome candidatos). É o **maior alavancamento** — fica a montante tanto da análise correta quanto do display honesto (L-C). **Plano detalhado:** `docs/plans/P2.4-proveniencia-por-campo.md` — 🔒 **retido para auditoria pré-execução** (gate da Fase A; decisões #A–D pré-confirmadas).
- **#P2-B** deixa de ser "regex-em-blob: sim/não" e passa a ser **"escopar ao produto: sim"** (casar nó por handle/SKU). Isso é o que de fato destrava os buckets difíceis com segurança (L-B). **Correção empírica (2026-06-17):** o mapeamento ao vivo do bucket "sem material" deu **NO-GO** — nenhuma loja-alvo tem a composição **só** num JSON island (tudo em HTML cru já é JSON-LD/prosa, coberto pela 1a); o gap real é **headless** (Zara/Adidas/Ralph Lauren, JS/XHR-only). Escopar island **não** destrava os buckets difíceis; **headless** sim. Ver `P2.4-proveniencia-por-campo.md §1b.0`.
- **L-A** é um ganho pequeno e imediato: realimentar o sinal de read na decisão de path (pular o reader quando o direct já está completo, mesmo `thin`); o enriquecimento por campo fica em P2.4. **Plano detalhado:** `docs/plans/L-A-read-signal-na-decisao.md`.

**L-C reforçado por N3 (G1, 2026-06-17): um chokepoint de normalização, não lógica por path.** A raiz do bug "ainda 2" do G1 foi que o dedup vivia no `extractText` (path direto) mas o `fetchViaReader` (path reader) tinha o **seu próprio** dedup, mais fraco (por URL exata), que **divergiu**. Mesma família do bug either/or original (Superdry) e do M4. Hoje os dois paths chamam `dedupeImages`, mas **ainda em dois lugares** — não num único limite. *Alvo:* identidade/normalização como **propriedade do candidato** (o modelo `{value, source, scope}` do L-C), deduplicada **uma vez no limite** (`fetchPage`/rota), não reimplementada por fonte. Tudo que deve valer para "o read" (dedup, identidade, proveniência, completude) pertence a um chokepoint compartilhado, senão os paths divergem.

---

> **Resumo:** P1 conserta o "analisar" (ausência≠julgamento). P2 conserta o "coletar" inteiro — source-merge + cobertura + **confiança-no-read** + proveniência por campo (Superdry é só uma instância). P5 conserta o "mostrar" (proveniência uniforme, depende de P2). P3/P4 são as Fases C/E com forma concreta (P3 consome o sinal de confiança do P2). **§6 (M1–M5 + OPS-1/OPS-2)** conserta o que está **ao redor** do motor: deploy, observabilidade, empty-states honestos, testes de fronteira e **confiar no harness** (carimbo de versão). **§7 (L-A/L-B/L-C + N3)** conserta o **interior**: realimentar o sinal de read na decisão (L-A), escopar fontes ao produto (L-B), tirar a perda da junção extração→parser (L-C) e normalizar num único chokepoint (N3). **G1 (2026-06-17)** acrescentou ainda M5/OPS-2 (confiar no harness + carimbo de versão) e o **7º princípio** (nunca inventar quantidade). Tudo amarrado pelos **7 princípios transversais**.
