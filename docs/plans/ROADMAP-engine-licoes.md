...
**OPS-2 — carimbo de versão (materializa M1 + M5) · a fazer.** Expor o **git SHA do build** (header `x-app-version` ou `/api/health`) e fazer **todo** check assertá-lo: `pnpm smoke`, curls manuais e a passada visual (Docker MCP) confirmam que estão batendo **no build pretendido**. **Um único sinal que pega os dois bugs** — o de prod (M1) e o do zumbi local (M5) — na hora, em vez de horas de depuração da lógica errada.
